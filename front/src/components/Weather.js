...
const Weather = () => {
    ...
    const openModal = (episodeId) => {
        console.log('Opening Modal for Episode ID:', episodeId);
            ...
            <svg x="55" y="105" width="165px" height="165px" viewBox="0 0 64 64">
                <defs>
                    <filter id="sun-shadow" x="-50%" y="-50%" width="250%" height="250%">
                        <feDropShadow dx="2" dy="2" stdDeviation="7" floodColor="#FFE8CC" floodOpacity="0.95" />
                    </filter>
            ...
            <svg x="35" y="90" width="187px" height="187px" viewBox="0 0 64 64">
            	<defs>
            	    <filter id="cloud-shadow" x="-50%" y="-50%" width="200%" height="200%">
                        <feDropShadow dx="2" dy="2" stdDeviation="3" floodColor="#9D9D9D" floodOpacity="0.25" />
                    </filter>
    ...
    const fetchRegionInfo = async (latitude, longitud) => {
        const REST_API_KEY = process.env.REACT_APP_API_KEY;
        const url = `https://dapi.kakao.com/v2/local/geo/coord2regioncode.json?x=${longitud}&y=${latitude}`;
        try {
            ...
            if (!response.ok) {
                throw new Error("Failed to fetch data from Kakao Maps API");
            }
