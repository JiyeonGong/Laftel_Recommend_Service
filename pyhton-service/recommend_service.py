...
def get_combined_recommendations():
   ...
   # 추천 목록을 기반으로 GPT 프롬프트 생성
    ...
    prompt = (f"현재 위치는 {city_name}이며, 날씨는 {current_weather}입니다. 사용자 성향은 {user_mbti}입니다." 
              f"이런 날씨와 성향에 맞는 애니메이션 추천은 다음과 같습니다: {anime_list}. 이 3개의 애니메이션들을 사용자에게 추천해주세요. 'Laftel에서 감상 가능한 애니 중 이런 작품은 어떠세요?' 로 문장을 시작하며 이 문장의 다음 줄에 추천을 해야합니다.\n"
              ...
              f"대화를 끝내기 전 추천한 애니메이션 리스트를 제목만 따로 출력하세요.\n"
    )
    try:
        # OpenAI API 호출
